...
# PHASE 9: CRYPTOGRAPHIC HASH-CHAINED BLACK BOX AUDIT LOGGER
class HashChainedAuditLog:
    ...
    def append_record(self, telemetry_payload):
        with self.lock:
            # Deterministic string conversion + link to previous record hash
    ...
    try:
        while True:
            ...
            if decrypted_bytes is None:
                print(
                    f"❌ DECRYPTION FAILED for {addr}! Invalid Key or Tampered"
                    " Data.\n"
                )
                continue
            ...
            with sequence_lock:
                ...
                if incoming_seq <= last_seq:
                    print(
                        f"⚠️ REPLAY/OUT-OF-ORDER ALERT! Node: [{engine_id}] |"
                        f" Dropped Packet ID: {incoming_seq}"
                    )
                    continue
            ...
            print(
                f"   [CSV Sensors] Temp: {temp}°C | Press: {press} PSI | RPM:"
                f" {rpm} | Vib: {vib} mm/s"
            )
            ...
            if temp > MAX_SAFE_TEMP_C:
                print(
                    f"   🚨 [ANOMALY] Thermal Spike Detected: {temp}°C"
                    f" (Limit: {MAX_SAFE_TEMP_C}°C)"
                )
                has_anomaly = True
            if vib > MAX_SAFE_VIB_MMS:
                print(
                    f"   🚨 [ANOMALY] Excessive Vibration: {vib} mm/s"
                    f" (Limit: {MAX_SAFE_VIB_MMS} mm/s)"
                )
                has_anomaly = True
            if rpm > MAX_SAFE_RPM:
                print(
...
def start_multi_node_receiver():
    ...
    ssl_context.verify_mode = (
        ssl.CERT_REQUIRED
    )  # Blocks unauthenticated socket connection attempts
    ...
    try:
        while True:
            ...
            try:
                # Wrap socket with mTLS handshake verification before thread creation
                tls_conn = ssl_context.wrap_socket(raw_conn, server_side=True)

                # Spawn a new background thread for each verified engine node
                ...
                print(
                    f"🚨 [PHASE 8 REJECTED] Unauthorized client connection"
                    f" blocked from {addr}: {e}\n"
                )
                raw_conn.close()
